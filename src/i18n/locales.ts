...
const zhTW = {
  header: {
    ...
    annotateMissingCount: (count: number) => `未講解剩餘：${count}`,
    annotateFailedCount: (count: number) => `重試失敗（${count}）`,
    annotateDisabled: "請先選擇講解來源",
  ...
  endpoint: {
    ...
    states: {
      ...
      ready: "已就緒",
      offline: "無法連線",
      "cors-blocked": "被瀏覽器擋下 (CORS)",
      "auth-missing": "缺少 API 金鑰",
      "model-missing": "模型不可用",
      "no-model": "已連線・尚無模型",
      "proxy-missing": "本地代理未啟動",
    } as Record<string, string>,
    defaultMsg: "準備探測端點…",
    recheck: "重新檢查",
    expand: "展開設定",
    ...
    endpoint: "端點網址",
    apiKey: "API 金鑰",
    apiKeyPlaceholder: "貼上金鑰（只留在本機記憶體，關頁即失）",
    apiKeyFromConfig: "已從 dit.config.json 讀入金鑰。",
    model: "模型",
    modelPlaceholder: "（連線後自動列出）",
    timeout: "逾時",
    sec: (n: number) => `${n} 秒`,
    privacyPolicy: "外傳保護",
    privacyBalanced: "平衡（預設）",
    privacyStrict: "嚴格",
    freeHint: "本地端點，講解不外傳。",
    costHint: "此端點可能消耗付費額度；首次講解會先跳出去識別化與同意確認。",
    corsBlockedGuide: "瀏覽器擋下了這個網址（CORS）。若這是雲端端點，通常代表它不支援瀏覽器直連——請改走「本地代理」預設。",
    authMissingGuide: "請先填入 API 金鑰。",
    proxyMissingGuide: "連不到本地代理，請確認代理伺服器已啟動。",
    noModelGuide: "已連線，但沒有可用的模型。",
    modelMissingGuide: "已連線，但指定的模型不存在；可從上方下拉改選。",
  },

  privacy: {
    eyebrow: "外傳前檢查",
    title: "確認送往外部講解服務的內容",
    body: "下方是本機去識別化後的實際送出文字。確認前不會呼叫任何外部服務 (INV-R8-1：任何外傳講解一律先過這關)。",
    policy: "政策",
    expires: "預覽有效至",
    preview: "處理後預覽",
    note: "本次同意只適用目前 session、講解來源端點／模型與隱私政策；疑似金鑰或密碼會直接阻擋，不能略過。",
    costNotice: "此講解來源可能消耗需付費的額度或訂閱；送出即代表你同意這次呼叫。",
    cancel: "取消",
    send: "送出去識別化內容",
  },
  ...
  providerDisclaimer: {
    ...
    cloud:
      "本地代理 (OpenCode)：DIT 先在本機去識別化並顯示實際送出預覽，確認後才透過本地代理傳給所選雲端模型；疑似金鑰或密碼一律阻擋。",
    lmstudio: "本地 LM Studio：講解由你機器上的模型產生，程式碼與紀錄不會外傳。",
    jan: "本地 Jan：講解由你機器上的模型產生，程式碼與紀錄不會外傳。",
    "anthropic-byok": "Anthropic（自帶金鑰，瀏覽器直連）：需要你自己的 API 金鑰，會消耗你的付費額度；送出前一律先過去識別化與同意關卡。",
    openrouter: "OpenRouter（瀏覽器直連）：需要你自己的 API 金鑰，會消耗你的額度；送出前一律先過去識別化與同意關卡。",
    groq: "Groq（瀏覽器直連）：需要你自己的 API 金鑰，會消耗你的額度；送出前一律先過去識別化與同意關卡。",
    custom: "自訂端點：填入任一 OpenAI 相容端點；若被瀏覽器 CORS 擋下，需改走本地代理。會消耗你的額度；送出前一律先過去識別化與同意關卡。",
  } as Record<ProviderId, string>,
...
const en: Messages = {
  header: {
    ...
    annotateMissingCount: (count: number) => `Not yet annotated: ${count}`,
    annotateFailedCount: (count: number) => `Retry failed (${count})`,
    annotateDisabled: "Select an annotation source first",
  ...
  endpoint: {
    panelLabel: "Endpoint settings",
    states: {
      checking: "Checking…",
      ready: "Ready",
      offline: "Cannot connect",
      "cors-blocked": "Blocked by the browser (CORS)",
      "auth-missing": "API key required",
      "model-missing": "Model unavailable",
      "no-model": "Connected, no models",
      "proxy-missing": "Local proxy not running",
    },
    defaultMsg: "Preparing to check the endpoint…",
    recheck: "Check again",
    expand: "Expand settings",
    collapse: "Collapse settings",
    endpoint: "Endpoint URL",
    apiKey: "API key",
    apiKeyPlaceholder: "Paste your key (kept in memory only, gone on page close)",
    apiKeyFromConfig: "Key loaded from dit.config.json.",
    model: "Model",
    modelPlaceholder: "(auto-filled once connected)",
    timeout: "Timeout",
    sec: (n: number) => `${n}s`,
    privacyPolicy: "Egress protection",
    privacyBalanced: "Balanced (default)",
    privacyStrict: "Strict",
    freeHint: "Local endpoint — nothing leaves your device.",
    costHint: "This endpoint may consume paid quota; the first annotation will ask for de-identification + consent confirmation.",
    corsBlockedGuide: "The browser blocked this URL (CORS). For a cloud endpoint, that usually means it doesn't support direct browser calls — use the \"local proxy\" preset instead.",
    authMissingGuide: "An API key is required.",
    proxyMissingGuide: "Cannot reach the local proxy — make sure the proxy server is running.",
    noModelGuide: "Connected, but no models are available.",
    modelMissingGuide: "Connected, but the specified model was not found; pick another from the dropdown above.",
  },

  privacy: {
    eyebrow: "Pre-egress review",
    title: "Confirm what the annotation endpoint will receive",
    body: "This is the exact locally de-identified text that will be sent. No external call happens before confirmation (INV-R8-1: every data-sending preset goes through this gate).",
    policy: "Policy",
    expires: "Preview expires",
    preview: "Sanitized preview",
    note: "Consent applies only to this session, the current endpoint/model, and privacy policy. Suspected keys or passwords are always blocked.",
    costNotice: "This endpoint may consume paid quota or a subscription; sending confirms you accept this call.",
    cancel: "Cancel",
  ...
  providerDisclaimer: {
    ...
    cloud:
      "Local proxy (OpenCode): DIT de-identifies content locally and previews the exact payload before the local proxy sends it to the selected cloud model. Suspected keys and passwords are always blocked.",
    lmstudio: "Local LM Studio: notes come from a model on your machine; code and logs are not sent out.",
    jan: "Local Jan: notes come from a model on your machine; code and logs are not sent out.",
    "anthropic-byok": "Anthropic (bring your own key, browser-direct): needs your own API key and consumes your paid quota; every call goes through de-identification and a consent gate first.",
    openrouter: "OpenRouter (browser-direct): needs your own API key and consumes your quota; every call goes through de-identification and a consent gate first.",
    groq: "Groq (browser-direct): needs your own API key and consumes your quota; every call goes through de-identification and a consent gate first.",
    custom: "Custom endpoint: point at any OpenAI-compatible endpoint; if the browser blocks it via CORS, route through the local proxy instead. Consumes your quota; every call goes through de-identification and a consent gate first.",
  },
