...
notes: User approved the sole-source contract `docs/PSM_R5_GUIDED_NAVIGATION_v1.0.md` on 2026-07-19. GN-01 through GN-09 are implemented as separate vertical slices. GN-09 restores Sidebar landmark marks and compact legends, readable global typography, subtle region layers, centered Map focus, a high-salience Close, endpoint-bounded spines, and typed fishbone geometry without changing approved interaction semantics. Latest GN-09 production UI evidence on the 50.0018 MiB / 29,452-view-item fixture: closed Reader DOM max 210, Map DOM max 397, first target 134 ms, and no horizontal overflow at 390/740/1280; the benchmark reports 18/18 checks pass. The unchanged ingest/cancel/deep-scroll evidence remains 964 ms load, first progress at 66 ms, 379 ms cancellation with the previous document preserved, and aligned deep index 28,541. Automated evidence is 130/130 tests plus typecheck, 117-module build, benchmark pass, and diff check. Final user visual and interaction UAT remains open, so this ticket intentionally stays in-progress.
