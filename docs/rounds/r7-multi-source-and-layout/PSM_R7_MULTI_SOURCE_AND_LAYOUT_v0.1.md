...
繼承且不得失守：[PSM_R5_GUIDED_NAVIGATION_v1.0.md](../r5-guided-navigation/PSM_R5_GUIDED_NAVIGATION_v1.0.md) §11、
[PSM_R5.5_SEMANTIC_ALIGNMENT_v0.1.md](../r5.5-semantic-alignment/PSM_R5.5_SEMANTIC_ALIGNMENT_v0.1.md) SA-INV-1～5、
[PSM_R6_EXPORT_v0.1.md](../r6-export/PSM_R6_EXPORT_v0.1.md) EX-INV-1～4、
[PSM_R6.5_LAYOUT_SCALE_REMEDIATION_v0.1.md](../r6.5-layout-scale/PSM_R6.5_LAYOUT_SCALE_REMEDIATION_v0.1.md) LS-INV-1～8。
本輪新增規範以 **R7-INV** 編號。
...
> **本節（連同 R7A-01／R7A-02）已於 2026-07-23 第三次修訂中整段棄用。** 三輪修正（等分 grid →
> flex-grow+basis → flex-grow+max-width → 本節的拿掉 grow 改內容決定寬度）處理的都是同一個模型
> ——「橫向塞進一行、靠斷點換行」——的變體，換行位置與留白永遠是視窗寬度的函數，不是能一次定案
> 的東西。使用者裁定改用對話框：群組垂直往下排，對話框寬度是設計常數，橫向排版類缺陷（RC-F 系列）
> 整類消失。技術方案見 [docs/rounds/r7-multi-source-and-layout/DESIGN_R7_SETTINGS_DIALOG_v0.1.md](DESIGN_R7_SETTINGS_DIALOG_v0.1.md)，
> 施工已完成（`src/components/SettingsDialog.tsx` ＋ `SettingsDialog.test.tsx`），取代下方
> R7A-01／R7A-02；**未另編 R7A 卡號**（R7A-06 已被字級 token 化卡佔用，見下方同名章節，避免
> 編號衝突）。以下三段（含 CSS 片段）保留作為
> 「為什麼放棄 flex-wrap 模型」的歷史記錄，**不再是現行實作**——`.settings-grid`／`.settings-group`
> 已從程式碼移除，改為 `.settings-dialog`／`.settings-panel-group`。
>
...
> **本節已被 R7A-00 量測推翻並修訂一次**，見 [docs/rounds/r7-multi-source-and-layout/R7_BASELINE_2026-07-23.md](R7_BASELINE_2026-07-23.md)。
...
> **本卡已棄用，改由設定對話框實作取代**（見 A4.1 開頭的第三次修訂說明）。以下內容為歷史記錄，
> 對應程式碼（`.settings-grid`／`.settings-group`／`g-*` class）已被移除，
> `Header.test.tsx` 相關斷言已搬到 `SettingsDialog.test.tsx`。
...
> **本卡已棄用，改由設定對話框實作取代**（見 A4.1 開頭的第三次修訂說明）。label/control 兩軌 grid 的結構
> 沿用進了 `SettingsDialog.tsx`，但控制欄改回 `minmax(0, 1fr)`——對話框寬度是固定常數，不會重演
> 本卡原本要避開的「控制欄撐滿群組、群組又撐滿控制欄」死白問題，`1fr` 在此安全。
...
> [docs/rounds/r7-multi-source-and-layout/R7_BASELINE_2026-07-23.md](R7_BASELINE_2026-07-23.md)。
- **檔案**：`src/styles/index.css`
...
- **檔案**：`docs/ACCEPTANCE.md`（§22）、`docs/PROGRESS.md`、`docs/BACKLOG.md`（R7 候選段標記完成）、
  `docs/rounds/r7-multi-source-and-layout/DESIGN_R7_MULTI_SOURCE_v0.1.md`（標註 §2／§6 已被本文件 §B1 實測修訂）、本文件
