...
- [ ] 設定匣可見「匯出」區塊，含隱私提示一行；390／740／1280 皆不水平溢位。
- [ ] 無 session 時「匯出 JSON」與「匯出 HTML 快照」皆為 disabled，不會產生空檔。
- [ ] 匯出 JSON：檔案可用文字編輯器開啟，最外層看得到 `ditExport` / `exportVersion` / `exportedAt`。
- [ ] 匯出 HTML 快照後**完全關閉 dev server／preview**，雙擊檔案開啟：Reader 可捲動、Map 可開可縮放、
      Overview 可見。
- [ ] 快照內看不到載入／講解／Provider／匯出入口；語言切換仍可用。
- [ ] 快照的瀏覽器 console 無錯誤，Network 分頁無任何對外請求。
- [ ] dev 模式（`npm.cmd run dev`）按「匯出 HTML 快照」時，出現明確提示而非產出壞檔。
- [ ] 匯出大型 session（如 `npm.cmd run fixture:r5` 產生的 50 MiB fixture）時，大小回報正確且提示
      檔案較大；瀏覽器未當掉。
- [ ] 既有能力無回歸：載入／取消、M 快捷鍵、Map Jump、cluster 不可跳、雙語切換、講解流程。
