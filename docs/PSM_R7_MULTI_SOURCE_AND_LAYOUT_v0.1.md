...
### A4.1 設定匣：需求導向流式配置（D-R7-01，R7-INV-1）
...
.settings-group {
  flex: var(--group-grow, 1) 1 var(--group-basis, 240px);
  min-width: min(100%, var(--group-basis, 240px));   /* 390 下退化成單欄，不溢位 */
  min-height: 0; margin: 0; padding: 0 12px 10px; border: 1px solid var(--rule);
}
.settings-group.g-language   { --group-basis: 170px; --group-grow: 0; }  /* 不被撐大 */
.settings-group.g-navigation { --group-basis: 260px; }
.settings-group.g-session    { --group-basis: 300px; }
.settings-group.g-export     { --group-basis: 300px; }
.settings-group.g-teaching   { --group-basis: 460px; --group-grow: 2; }  /* 需求最大，優先吃剩餘空間 */
```

**演算法效果**（1280 可用寬約 1244）：`170 + 260 + 300 + 300 + 460 = 1490 > 1244` → 換行成兩列；
教學講解（basis 460）獨佔第一列大部分，語言（grow 0）維持 170px 不再空轉。
740 下自然收成兩欄、390 下每組滿寬——**同一組規則，無需任何斷點**。
...
### R7A-01 — 設定匣需求導向配置（RC-F(a)，R7-INV-1）

- **檔案**：`src/styles/index.css`、`src/components/Header.tsx`、`src/components/ExportControls.tsx`
- **契約**：A4.1 的容器規則與五組 `--group-basis` / `--group-grow` 值。
- **做什麼**：`.settings-grid` 由 grid 改 flex-wrap；`.settings-group` 加 `flex` / `min-width`；
  四個 fieldset 加 `g-session` / `g-teaching` / `g-language` / `g-navigation` class，
  `ExportControls` 的 fieldset 加 `g-export`。
- **錯誤路徑**：未帶 `g-*` class 的群組（未來新增者）落到 `--group-basis: 240px` 預設值，
  不會破版——預設值為必要的安全網，不得移除。
...
- **驗收證據**：390 / 740 / 1280 / 1706 × zh / en 共 8 張設定匣截圖；1280 下語言組
  `clientWidth ≤ 200px`、教學講解組 `clientWidth ≥ 440px`。

### R7A-02 — 教學講解群組內部分層（RC-F(b)，R7-INV-1）

- **檔案**：`src/components/Header.tsx`、`src/styles/index.css`
- **契約**：A4.1 的 `.settings-actions.rows` 兩軌 grid 與四列結構表。
