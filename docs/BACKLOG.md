...
- [ ] **R10-A Codex 保真度**：upstream `codex-rs/rollout/src/policy.rs` 明載 rollout 有 `Legacy` 與
...
- [ ] **R10-B Source Profile 側寫化**：`detectAdapter()` 有正確辨識來源、`doc.session.source` 也有記錄，
...
- [ ] **R10-C Session 內全文搜尋**：競品掃描顯示這是唯一一項「競品全有、DIT 全無、且與教學定位相容」
