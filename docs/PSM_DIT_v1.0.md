...
**R5 — 大檔虛擬化 + 響應式**
- 🟡 2026-07-19 已實作並通過自動化／效能預檢；原始 1–7 驗收通過後依使用者截圖完成資訊架構修正，等待最新畫面最終驗收。
- 範圍：`@tanstack/react-virtual` 分別虛擬化 Sidebar／MainView；production `File.text()` 改為
  ...
  所有寬度採預設收合設定匣；閱讀／魚骨／子代理／結構為互斥工作區，只掛載一個 panel；魚骨目前
  station 的 ribs 與子代理摘要清單各自虛擬化，導覽選取會停止播放並回閱讀定位。
- 驗收：固定 50.0018 MiB fixture 由 70,993 ms 降為 1,389 ms；高密度頁面總 DOM 383,276→240，
  最新 390×844 四工作區 DOM 為 129／116／137／128，深層選取／下一步／子代理播放優先序正確；
  95/95 tests、typecheck、build、`git diff --check` 通過。詳細量測見
...
| 024 | 2026-07-19 | ADR-023 的全攤開版面由預設收合設定匣＋閱讀／魚骨／子代理／結構互斥工作區取代；魚骨 ribs 與子代理清單有界渲染 | 使用者以實機截圖指出資訊仍擠壓內容並拍板 D1–D3；一次只看一種資訊，保留同一 ViewItem 選取語意 |
