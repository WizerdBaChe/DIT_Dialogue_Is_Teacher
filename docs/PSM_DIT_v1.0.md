...
- 範圍：Vitest；(1) pipeline 快照測試——`src/fixtures/sampleSession.jsonl` 經
  adapter→normalize→denoise→distill 的輸出快照；(2) denoiser 規則單元測試（5 條規則各至少一正一反例）；
  (3) distiller 規則單元測試（spine/rib 各 kind）；(4) 容錯測試——損壞行/未知型別/空檔案不崩、warnings 正確。
- 不做：元件測試、E2E（本階段收益低）。
- 驗收：`npm test` 全綠並納入 build 流程說明；快照檔進版控。
- 新增第二個 fixture：含 subagent(isSidechain)/長輸出/多任務分界的真實 session 片段，為 R4 鋪路。
