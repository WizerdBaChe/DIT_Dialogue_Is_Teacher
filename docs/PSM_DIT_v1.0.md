...
> - R5 guided navigation 的已批准施工語意以 [PSM_R5_GUIDED_NAVIGATION_v1.0.md](rounds/r5-guided-navigation/PSM_R5_GUIDED_NAVIGATION_v1.0.md)
>   為唯一依據；本文件只同步其 as-built 結果與 ADR，不重新定義互動。
...
- `opencode serve` 必須由專案根目錄啟動且不得加 `--pure`，才能載入 `opencode.json` 的
  ...
  [PSM_PRIVACY_GATEWAY_v0.1.md](rounds/r3-analysis-runtime/PSM_PRIVACY_GATEWAY_v0.1.md) 與
  [PSM_DIT_ANALYSIS_RUNTIME_v0.1.md](rounds/r3-analysis-runtime/PSM_DIT_ANALYSIS_RUNTIME_v0.1.md)。
...
- ✅ 已完成（2026-07-18）：2026-07-17 的 `qwen2.5:3b` 首輪 6/10 基線保留；`qwen2.5-coder:7b` 重跑為 7/10，內容門檻與無崩潰通過。timeout／disconnect 文案通過自動化測試，使用者另以工作管理員終止／重啟 `ollama.exe`，確認 UI 錯誤與重新檢查恢復正常。證據見 [UAT_ollama_2026-07-18.md](rounds/r2-ollama-uat/UAT_ollama_2026-07-18.md)。
...
**R5 — 大檔虛擬化 + 響應式**
- 🟡 2026-07-19 GN-01～GN-08 已實作並通過自動化／production preview 預檢；等待使用者完成最終視覺 UAT。
- 範圍：`@tanstack/react-virtual` 分別虛擬化 Sidebar／MainView；production `File.text()` 改為
  Worker + Blob streaming；reading/parsing/organizing/validating/ready 進度、取消與原子發布；
  顯式 `PrimaryView`／`SessionOrigin`、guided Overview、≥720 persistent Structure、<720 native drawer、
  Reader Minimap、native Session Map、deterministic semantic zoom 與安全 `M`；導覽選取停止播放並回 Reader。
- 驗收：固定 50.0018 MiB fixture 在最新 production preview 於 964 ms 載入；Reader closed DOM 最大 249，
  global／section／detail Map 最大 DOM 474／477／354，首個 target 115 ms；390／740／1280 無水平溢出，
  深層 index 28,541 的 Sidebar／Reader 同步；128/128 tests、typecheck、build、`git diff --check` 通過。詳細量測見
  [R5_BENCHMARK_2026-07-19.md](rounds/r5-guided-navigation/R5_BENCHMARK_2026-07-19.md)；視覺完成仍待使用者確認。
...
| 022 | 2026-07-19 | R5 清單虛擬化採 `@tanstack/react-virtual`，Sidebar 與 MainView 各自滾動、以 ViewItem ID 橋接未掛載選取 | 使用者拍板；避免自行維護動態高度與 remeasurement 邊界 |
| 023 | 2026-07-19 | 窄螢幕採 inline 可收合結構面板、主內容全寬、魚骨水平捲動、詳情置於圖下 | 使用者拍板；保留所有資料與既有魚骨認知語意 |
| 024 | 2026-07-19 | **superseded**：ADR-023 的全攤開版面改為預設收合設定匣與四個互斥 workspace | 使用者後續否決 Structure 作為 tab 與 unguided Reader entry；由 ADR-025 取代 |
| 025 | 2026-07-19 | **approved**：顯式 Overview／Reader／Subagents 主視角；≥720 persistent Structure、<720 drawer；Reader Minimap＋modal Session Map；安全 `M` | 使用者批准 D4–D6、G1–G4 與 [PSM_R5_GUIDED_NAVIGATION_v1.0.md](rounds/r5-guided-navigation/PSM_R5_GUIDED_NAVIGATION_v1.0.md)；Map 使用 deterministic global／section／detail caps |
...
| v1.2 | 2026-07-04 | ADR-014（repo 於現址 git init、不 archive）；新增 docs/rounds/r1-test-foundation/IMPLEMENTATION_KICKOFF_PROMPT.md 作為實作 session 開場 |
